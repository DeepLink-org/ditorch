...
欢迎加入 ditorch 社区，我们致力于打造训练框架和人工智能芯片之间的标准算子接口，我们欢迎任何类型的贡献，包括但不限于
...
```shell
git clone git@github.com:{username}/ditorch.git
```

添加原代码库为上游代码库

```bash
git remote add upstream git@github.com:DeepLink-org/ditorch
```

检查 remote 是否添加成功，在终端输入 `git remote -v`

```bash
origin	git@github.com:{username}/ditorch.git (fetch)
origin	git@github.com:{username}/ditorch.git (push)
upstream	git@github.com:DeepLink-org/ditorch (fetch)
upstream	git@github.com:DeepLink-org/ditorch (push)
```

> 这里对 origin 和 upstream 进行一个简单的介绍，当我们使用 git clone 来克隆代码时，会默认创建一个 origin 的 remote，它指向我们克隆的代码库地址，而 upstream 则是我们自己添加的，用来指向原始代码库地址。当然如果你不喜欢他叫 upstream，也可以自己修改。我们通常向 origin 提交代码（即 fork 下来的远程仓库），然后向 upstream 提交一个 pull request。如果提交的代码和最新的代码发生冲突，再从 upstream 拉取最新的代码，和本地分支解决冲突，再提交到 origin。
...
#### 3. 提交代码并在本地通过一致性测试
提交的代码需要通过一致性测试套件以保证实现的正确性，具体可以参考一致性测试套件的[README](https://github.com/DeepLink-org/ditorch)。
...
(2) 如果是第一次为 ditorch 做贡献，需要签署 CLA
<!--
（图片待修改）
